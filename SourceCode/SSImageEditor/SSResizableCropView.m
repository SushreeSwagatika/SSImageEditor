...
@implementation SSResizableCropView
@dynamic crop;
@dynamic image;
...
- (id)initWithImage:(UIImage*)newImage andSize:(CGSize)maxSize andStartCoordinate:(CGPoint)coordinateLocation {
    self = [super init];
    if (self) {
        self.frame = [self calcFrameWithImage:newImage andMaxSize:maxSize andLocation:coordinateLocation];
...
- (void)setup {
    ...
    cropView = [SSResizableCropView initialCropViewForImageView:imageView];
...
+ (UIView *)initialCropViewForImageView:(UIImageView*)imageView {
    ...
    CGFloat width  = CGWidth(max) / 4 * 1;
    CGFloat height = CGHeight(max) / 4 * 1;
...
- (CGRect)calcFrameWithImage:(UIImage*)image andMaxSize:(CGSize)maxSize andLocation:(CGPoint)loc{
    CGFloat increase = IMAGE_CROPPER_OUTSIDE_STILL_TOUCHABLE * 2;
    
    // if it already fits, return that
    CGRect noScale = CGRectMake(loc.x, loc.y, image.size.width + increase, image.size.height + increase);
...
#pragma mark - Tap Gesture for UIImageView

- (void)imageTap
{
    UITapGestureRecognizer *singleTapp = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapDetected)];
    singleTapp.numberOfTapsRequired = 1;
    [imageView setUserInteractionEnabled:YES];
    [imageView addGestureRecognizer:singleTapp];
}

-(void)tapDetected
{
    [cropView setHidden:0];
}
